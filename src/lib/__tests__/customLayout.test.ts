...
describe('computeCellsFromLines', () => {
  ...
  it('creates three horizontal bands with two horizontal lines', () => {
    ...
    expect(cells).toContainEqual({ x: 0, y: 0.66, width: 1, height: 0.33999999999999997 })
  })

  it('treats partial lines as interior walls (regions flow around them)', () => {
    // Partial vertical line at x=0.5 from y=0 to y=0.5.
    // Since the line does NOT span the full board height, the flood-fill
    // finds a path around it through the bottom half → everything connects.
    const cells = computeCellsFromLines([
      { id: '1', type: 'vertical', position: 0.5, start: 0, end: 0.5 },
    ])
    // One big cell: the partial line does not partition the board.
    expect(cells).toHaveLength(1)
    expect(cells[0]).toEqual({ x: 0, y: 0, width: 1, height: 1 })
  })

  it('creates three cells with a T-shaped line arrangement', () => {
    // Vertical full line + horizontal partial line → T junction
    // Creates: left-top, right-top, and bottom (one big cell)
    const cells = computeCellsFromLines([
      { id: '1', type: 'vertical', position: 0.5 },
      { id: '2', type: 'horizontal', position: 0.5, start: 0.5, end: 1 },
    ])
